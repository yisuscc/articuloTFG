...
distribution across phases is assumed. The aforementioned PMIC also excludes the cooler and USB peripherals, so E0--E4 values are lower
...
twofold: a smaller footprint reduces per-token memory traffic, raising throughput and energy
...
idle period before inference, so the resulting $\varepsilon$ values are upper
