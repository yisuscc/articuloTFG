...
by up to 53\% and renders four of the six models non-interactive. We also found that 4-bit
quantization is the optimal trade-off for the Raspberry Pi 5.
...
\begin{itemize}
  \item \textbf{A reusable, open-source measurement infrastructure.}
    ...
    reproducible inference experiments at the egde while simultaneously capturing
  ...
  \item \textbf{A six-factor experimental characterization.} Six experimental
    ...
    the Raspberry~Pi AI~HAT+~2, totaling 113 distinct instrumented runs
  ...
  \item \textbf{Actionable deployment guidance.} Active cooling is an
    ...
    the optimum, with quality degradation below
...
parallel and is theoretically compute-bound, and a \emph{decode} phase that
generates one token at a time and is theoretically bound by memory bandwidth,
given that every step must stream the model weights and the growing key--value (KV)
cache through the memory hierarchy~\cite{whatmough_generative_2025}.
 On inference, this second regime dominates in temporal extension~\cite{li_large_2025}, which makes weight compression the
...
E0 series confirms and quantifies for generative workloads. More recently,
Dangwal et al.~\cite{dangwal_reliable_2026} likewise treat frequency
throttling as a first-order factor in edge deployments, although in their case, it is
induced by undervoltage on shared power supplies rather than by temperature;
they show that explicitly modeling this variability, rather than attempting to
eliminate it, provides a practical and robust foundation for performance
prediction.
...
quantization, and power modes; their study uses
...
Hardware resources monitoring; thermal behavior &
SLM inference \\
\addlinespace[2pt]
...
Multiple engines; hardware resources monitoring; normalized energy (J/token); model diversity (one model) \\
\addlinespace[2pt]
Arya and Simmhan~\cite{11105796} &
Partial hardware resources monitoring (incl. power modes); absolute energy (J); quantization;
batch size &
Specialized engines; normalized energy (J/token) \\
\addlinespace[2pt]
Sobhani et al.~\cite{sobhani_sustainability_2025} &
Partial hardware resources monitoring (power, memory); generative quality (F1);
multiple platforms &
...
Multiple engines (CPU only); hardware resources monitoring; thermal behavior \\
\midrule
\textbf{This work} &
...
Generative quality (task-based); multiple hardware platforms \\
\bottomrule
\end{tabular}
\end{table}
...
evaluation, and commercial accelerators. This work addresses this gap with
...
This subsection defines the metrics studied in this work, classified in six domains.

\paragraph{Inference performance.}
Generation \emph{throughput} $T$ (tokens/s) in the decode phase (throughout the
paper, uppercase $T$ denotes throughput and lowercase $t$ denotes execution
time);
...
invocation and the start of the prefill phase.
...
\bar{P}_{\mathrm{run}}\,t_{\mathrm{run}}$, and our comparison metric, energy efficiency, is calculated as $\varepsilon = T/\bar{P}_{\mathrm{run}}$ (tokens/J); the energy per token (J/tokens) is its
...
distribution across phases is assumed. The aforementioned PMIC reports power only and also excludes the cooler and USB peripherals, so the measured power is underestimated and the resulting $\varepsilon$ values of E0--E4 are upper
...
$W_{\mathrm{CPU}}$ is expressed in nominal core-seconds (core$\cdot$s); thus, the compute
...
$\mathrm{MBU} = (M_{\mathrm{model}} + M_{\mathrm{KV}})\,T / BW_{\mathrm{peak}}$,
where $T$ is the decode throughput defined above (its inverse, $1/T$, is the
mean time per output token) and the $M$ terms denote the size in bytes: $M_{\mathrm{model}} = N_{\mathrm{params}}\,\mathrm{bpw}/8$
...
\begin{equation}
  ...
  \left(M_{\mathrm{model}}^{\mathrm{act}} + M_{\mathrm{KV}}^{\mathrm{attn}} +
  M_{\mathrm{state}}\right) T,
  \label{eq:mbu_corr}
\end{equation}
...
weighted geometric mean $\mathrm{FoM}_{\mathrm{full}}$, which drives the score to zero if any axis collapses
...
\begin{equation}
  ...
  \qquad
  \tilde{T}_{5} = \frac{T}{T_{\mathrm{cpu}}},\qquad
  \tilde{\varepsilon}_{5} = \frac{\varepsilon}{\varepsilon_{\mathrm{cpu}}} ,
  \label{eq:fom_red}
\end{equation}
...
each configuration is judged against its own threshold (tok/s); the applicable
...
Each run produces three data structures (per-prompt inference metrics in JSONL,
periodic hardware samples in JSONL, and a summary with the full configuration
and model metadata in JSON) in a directory marked by a nanosecond timestamp.
Reproducibility is guaranteed by pinning the random seed and sampling temperature:
under identical hardware,engine, and model versions, outputs are deterministic.
...
DataFrames. The resulting datasets generated in this study containing inference and hardware metrics alongside the metadata are publicly
...
function of SoC temperature. For series E5 the platform was extended with the Raspberry~Pi
...
2026-04-21 image), without a graphical environment to minimize background
interference, and with PCIe~Gen~3 enabled in the firmware EEPROM for the
...
$^{\dagger}$~Only the six Ollama runs are additional: the llama.cpp side of E4 (six runs) reuses the actively-cooled reference runs of E0.}
\label{tab:matrix}
\small
...
E4 & RQ4 & Inference engine & llama.cpp vs.\ Ollama & 6$^{\dagger}$ \\
E5 & RQ5 & Accelerator & CPU (llama.cpp) vs.\ Hailo-10H (hailo-ollama) & 4 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Baseline characterization (Series E0) of the six models in the reference
...
count; the $\mathrm{FoM}_{\mathrm{full}}$ is defined in Eq.~\eqref{eq:fom_full}. MBU is the corrected
...
descending $\mathrm{FoM}_{\mathrm{full}}$ within each cooling condition.}
\label{tab:e0}
...
threshold. The $\mathrm{FoM}_{\mathrm{full}}$ places Ministral~3B first
...
frontier; the marker size is proportional to $\mathrm{FoM}_\mathrm{full}$. Ministral~3B, despite
presenting the lowest speed among usable leaders and the largest footprint (3.01~GByte),
attains the highest $\mathrm{FoM}_\mathrm{full}$ as a result of the intrinsic weighting encoded by this figure of merit, in terms of the number of active parameters.
...
axis; differentiation comes from throughput, corrected bandwidth utilization and weighted
energy efficiency (Figure~\ref{fig:e0_fom}). The platform operates
during decoding with an $\mathrm{MBU}_{\mathrm{corr}}$ between 53
and 66\%.
...
maximum corresponding to Llama~3.2~3B, the largest model of the set).
Values reported per phase correspond to per-prompt averages.
 Given the pronounced right skew of these distributions, 
 the relative ranking of models varies with the statistic chosen:
under the median, DeepSeek-R1 exhibits the highest decoding cost (496~s, versus 133~s for Llama~3.2~3B),
a consequence of its reasoning traces lengthening nearly every output. By contrast, Llama~3.2~3B's mean is disproportionately 
shaped by just four of its fifteen prompts exceeding 1300~s. Given that decoding dominates, any
thermally induced frequency reduction translates proportionally into global
throughput loss (Figure~\ref{fig:e0_cooling}), which explains the four models that become out of the
...
reduces the memory footprint but \emph{improves} the $\mathrm{FoM}_{\mathrm{full}}$ in every
model: $\mathrm{FoM}_{\mathrm{full}}$ grows from Q8\_0 towards
Q4 and Q3, peaking at Q4\_0 or Q3\_K\_M depending on the model (Figure~\ref{fig:e1_fom_bpw}). The cause is
is that a smaller footprint reduces per-token memory traffic, raising throughput and energy
...
Llama~3.2~1B to 1.6~tok/s. Excess precision is therefore counterproductive.
...
horizontal line marks the reference level ($\mathrm{FoM}_{\mathrm{full}}=1$); stars mark
...
In the quality dimension (Figure~\ref{fig:e1_ppl}), the perplexity degradation from
...
to $+13.4\%$ in Llama-3.2-1B), although it loads faster, with a lower TTFT
in all models (e.g., 1164 vs.\ 1874~ms in Ministral-3B). These
...
if only the $\mathrm{FoM}_{\mathrm{full}}$ is considered, Q4\_0 and Q3\_K\_M are the best alternatives:
Q3\_K\_M attains the highest $\mathrm{FoM}_{\mathrm{full}}$ in four of the six models and Q4\_0 in
Ministral-3B, and both outperform Q4\_K\_M in $\mathrm{FoM}_{\mathrm{full}}$ in every model except
Llama-3.2-1B (e.g., Llama-3.2-1B at Q3\_K\_M runs in 0.97~GByte at
11.4~tok/s). However, once quality degradation is also taken into account,
Q4\_K\_M offers the best balance: its $\mathrm{FoM}_{\mathrm{full}}$ is close to the maximum, it is usable
in all six models, and its perplexity stays within 5\% of Q8\_0, whereas Q4\_0 and
Q3\_K\_M degrade it by up to 13\% and 19\%, respectively. Q4\_K\_M is therefore
recommended as the default operating point.
...
$+21\%$ (Llama~3.2~3B) over the full range (Figure~\ref{fig:e2_ram}), never
exceeding 3.2~GByte; context
...
is to the growing per-token KV-cache traffic. Gemma~3n and Granite, whose MBU
stays close to or below ${\approx}55\%$ across the range, are practically
insensitive. No model crosses the usability
threshold within the range. The composite figure of merit follows the same
pattern (Figure~\ref{fig:e2_fom}): it falls progressively in the
bandwidth-bound models (DeepSeek from 1.164 to 1.002, $-14\%$, and
Llama-3.2-3B from 1.070 to 0.983, $-8\%$), while Gemma~3n and Granite stay
flat, confirming that around ${\approx}55\%$ MBU the context ceases to be the
limiting factor. The practical recommendation is to size the
context to the smallest value that covers the application's needs.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/e2_ram.png}
\caption{Measured runtime RAM footprint versus context-window size, one line
per model. The slope reflects the KV-cache contribution: standard-attention
models grow by 8--21\% across the range, while the hybrid SSM of Granite, whose
state is fixed-size, stays flat ($+2\%$). No configuration approaches the
platform's 8~GByte limit. Active cooling.}
\label{fig:e2_ram}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/e2_fom.png}
\caption{$\mathrm{FoM}_{\mathrm{full}}$ versus context-window size, one line
per model. The dashed line marks the reference value $\mathrm{FoM}=1$. Models
with high bandwidth utilization lose up to 14\% across the range, whereas
Gemma~3n and Granite are essentially insensitive. Active cooling.}
\label{fig:e2_fom}
\end{figure}
...
metric: RAM, throughput, TTFT, energy per token, and $\mathrm{FoM}_{\mathrm{full}}$ remain constant
...
The llama.cpp runs correspond to the actively-cooled reference runs of series E0 and
were not repeated; only the six Ollama runs are additional.
Table~\ref{tab:e4} reports the results.

\begin{table}[!t]
\centering
\caption{llama.cpp (l.cpp) versus Ollama per model, with active cooling. The l.cpp columns are the actively-cooled
reference runs of series E0 (Table~\ref{tab:e0}).
Positive $\Delta$ values indicate a llama.cpp advantage and negative values an Ollama advantage. \emph{Us.} marks the
...
\multicolumn{2}{c}{\textbf{$\varepsilon$ (tok/J)}} & \multicolumn{2}{c}{\textbf{$\mathrm{FoM}_{\mathrm{full}}$}} &
...
\textbf{$T$} & \textbf{$\mathrm{FoM}_{\mathrm{full}}$} & \textbf{l.cpp/Oll.} \\
\midrule
...
Eq.~\eqref{eq:fom_red} ($\mathrm{FoM}_{\mathrm{red}}$). Unlike series E0--E4, energy was read from an
external wall-power meter for \emph{both} substrates (CPU/Hailo), capturing the total power consumption, including that of the HAT. The reading includes boot and ${\sim}5$~minutes of
idle period before inference, so the resulting $\varepsilon$ values are lower
bounds, comparable within the series but not with E0--E4. Only two models distributed by Hailo in
...
on both axes: throughput and energy efficiency ($\mathrm{FoM}_{\mathrm{red}}=0.822$). For Llama-3.2-3B the
...
models and the context/batch parameters fixed at compilation time)
for the efficiency
...
\paragraph{Decoding regime and memory bandwidth utilization.}% rewrite and reprhase w/o memory-bound
Under nominal decoding conditions (E0 with fan), the target platform exhibits an
$\mathrm{MBU}_{\mathrm{corr}}$ between 53 and 66\%. Separately, four experimental
series characterize the system dynamics: in E1, throughput rises as weight
precision drops from 8 to ${\approx}4$~bpw, because lighter weights move less data
per token, leveling off below Q5\_K\_M in Llama-3.2-1B; in E2, larger context
windows increase KV-cache traffic and degrade throughput by up to 22\% in the
largest models; in E3, prefill batch size is irrelevant because single-stream
decoding finds no additional parallelism; and in E4, llama.cpp's serverless
architecture reduces first-token latency without altering decoding throughput.
...
models usable and, with Q4\_K\_M, degrades perplexity by less than 5\% with
respect to Q8\_0, offering the best balance between $\mathrm{FoM}_{\mathrm{full}}$ and quality.
Q4\_0 and Q3\_K\_M reach the highest $\mathrm{FoM}_{\mathrm{full}}$ and can substitute Q4\_K\_M when
latency or memory dominate, at the cost of higher quality degradation.
...
models non-interactive; (ii)~4-bit quantization is the optimum on this
 platform, with Q4\_K\_M as the best balance between $\mathrm{FoM}_{\mathrm{full}}$ and
quality (Q4\_0 and Q3\_K\_M reach a higher $\mathrm{FoM}_{\mathrm{full}}$), whereas 8-bit and full-precision formats are
...
cooler and peripherals (power is underestimated), and its measurement protocol differs
...
%% TODO: add \Description{...} (alt text, required by acmart for accessibility)
%% to the figures flagged as "possible image without description" on the
%% \includegraphics at lines ~750, 761, 776, 799, 917, 928 and 978.
