...
\paragraph{Memory bandwidth is the dominant limiting factor.}% rewrite and reprhase w/o memory-bound
The target platform operates during decoding, with an
$\mathrm{MBU}_{\mathrm{corr}}$ between 53 and 66\% in nominal conditions.
This single bottleneck explains four series: in E1, throughput rises
as weight precision drops from 8 to ${\approx}4$~bpw, because lighter weights
move less memory per token, and the gain levels off below Q5\_K\_M in
Llama-3.2-1B; in E2, larger context windows increase KV-cache traffic
and degrade throughput by up to 22\% in the models with the highest MBU; in
